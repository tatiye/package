...
$NEWTOKEN="eyJkaXIiOiJwb3N0aW5nYW5cL0FwaVwvMC4xXC9saXN0LnBocCIsInVpZCI6MX0";
...
$QUERY="SELECT 
        *
        FROM appnews
        WHERE segment='Tatiye' $search ORDER BY dilihat DESC ".$data["record"];
...
$result=$db->query($QUERY);
 while($row=$result->fetch_assoc()){
  $no=$no+1;
    ...
    // $IMGS= tatiye::fetch('appfile','filename,date',"keyid='".$row['id']."' AND nmtabel='appnews' AND categori='images' ORDER BY id DESC");
  ...
  $number=array("no"=>$no);
    ...
    $sub_array["thumbnail"] =tatiye::fileimg([$row['id'],'appnews','300x215']);      

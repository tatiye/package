...
$NEWTOKEN="eyJkaXIiOiJkZXZpY2VzXC9BcGlcLzAuMVwvYXJjaGl2ZS5waHAiLCJ1aWQiOjF9";
Authorization::init(1);
 ...
 $setUId=$_SESSION['user_id'] ;
  ...
  if (!empty($_SERVER["HTTP_KEY"])) {
   $categori='label_'.$_SERVER["HTTP_KEY"];
 } else {
   $categori='archive';
 }
if($_SERVER["REQUEST_METHOD"] === "GET") {
  $QUERY="SELECT 
    *
    FROM apparchive
        WHERE $setRow AND categori='".$categori."' AND package !='office' ORDER BY id DESC";
$COUNT=tatiye::fetch("apparchive"," COUNT(*) as count","package !='office'");
  $number=0;                                        
  ...
  while ($row = $variable->fetch()) {               
    ...
    $var = json_decode($row['arsip'], true);                            
    $Expuid=tatiye::fetchUserIDTabel($var["userid"]);                                              
    $sub_array = array();                                               
    $sub_array[] =$number;                                              

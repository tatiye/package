...
$NEWTOKEN="eyJkaXIiOiJkZXZpY2VzXC9BcGlcLzAuMVwvZm9sZGVyLnBocCIsInVpZCI6MX0";
if($_SERVER["REQUEST_METHOD"] === "POST") {
$val=json_decode(file_get_contents("php://input"));
$db=new tatiye();
$tabel="appfile";
$keywords="nama,keyid,categori,nmtabel,fileType,filename";
$COUNT=tatiye::fetch("appfile"," COUNT(*) as count");
...
$QUERY="SELECT 
        *,COUNT(*) as count
        FROM appfile
        WHERE row='1' GROUP BY bulan $search ".$data["record"];
...
$result=$db->query($QUERY);
 while($row=$result->fetch_assoc()){
  ...
  $number=array("no"=>$no);       
    ...
    $sub_array["data"] =$row["count"].' files';  
    $sub_array["folder"] =tatiye::month($row["bulan"]);  

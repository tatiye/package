...
$NEWTOKEN="eyJkaXIiOiJwcmljaW5nXC9BcGlcLzAuMVwvc2VsZWN0LnBocCIsInVpZCI6MX0";
...
function item($key){
    ...
    $resultItems =tatiye::sqli("SELECT nama FROM salesitem WHERE idprice='".$key."'");
     while ($item = $resultItems->fetch_array()) {  
      $no=$no+1;
      $str=$str.'<li>'.$no.'.'.$item['nama'].'</li>';
    } 
     return '<ol class="list-unstyled lh-7">'.$str.'</ol>';  
}
/* and class item */
$QUERY="SELECT 
        *
